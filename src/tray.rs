...
fn make_tray(in_server_process: bool) -> hbb_common::ResultType<()> {
    ...
    let stop_service_label = if in_server_process {
        "Exit"
    } else {
        "Stop service"
    };
    let stop_service_i = MenuItem::new(translate(stop_service_label.to_owned()), true, None);
    let quit_tray_i = MenuItem::new(translate("Exit tray".to_owned()), true, None);
    let open_i = MenuItem::new(translate("Open".to_owned()), true, None);
    if in_server_process {
        tray_menu.append_items(&[&open_i, &stop_service_i]).ok();
    } else {
        tray_menu
            .append_items(&[&open_i, &quit_tray_i, &stop_service_i])
            .ok();
    };
        ...
        *control_flow = ControlFlow::WaitUntil(
        ...
        if let Ok(event) = menu_channel.try_recv() {
            if event.id == stop_service_i.id() {
                ...
                let mut is_service_running = in_server_process;
                if !is_service_running {
                    #[cfg(target_os = "windows")]
                    {
                        is_service_running = crate::platform::is_self_service_running();
                    }
                    #[cfg(target_os = "linux")]
                    {
                        is_service_running = crate::check_process("--server", false)
                    }
                }
                if !is_service_running {
            ...
            } else if event.id == quit_tray_i.id() {
                *control_flow = ControlFlow::Exit;
